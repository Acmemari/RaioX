...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    if (!import.meta.env.VITE_SUPABASE_URL || !import.meta.env.VITE_SUPABASE_ANON_KEY) {
      setEnvError('Variáveis de ambiente não configuradas. Por favor, configure VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY no arquivo .env.local');
    }
  }, []);
  ...
  if (envError) {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-ai-bg text-ai-text p-4">
        <div className="max-w-md w-full bg-white rounded-xl border border-ai-border shadow-sm p-6">
          <h2 className="text-lg font-semibold text-ai-text mb-2">Erro de Configuração</h2>
          <p className="text-sm text-ai-subtext mb-4">{envError}</p>
          <p className="text-xs text-ai-subtext">
            Crie um arquivo <code className="bg-ai-surface px-1 py-0.5 rounded">.env.local</code> na raiz do projeto com as variáveis necessárias.
          </p>
        </div>
      </div>
    );
