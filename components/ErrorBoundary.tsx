...
class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      // Detectar erros relacionados a variáveis de ambiente
      const isEnvError = this.state.error?.message?.includes('Variáveis de ambiente') || 
                        this.state.error?.message?.includes('VITE_SUPABASE');

      return (
        <div className="flex flex-col items-center justify-center h-full p-8 bg-ai-surface min-h-screen">
          <div className="bg-white rounded-xl border border-ai-border shadow-sm p-8 max-w-md w-full">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-ai-text">
                {isEnvError ? 'Erro de Configuração' : 'Algo deu errado'}
              </h2>
            </div>
            <p className="text-sm text-ai-subtext mb-6">
              {isEnvError 
                ? 'As variáveis de ambiente não estão configuradas corretamente. Por favor, verifique o arquivo .env.local e certifique-se de que VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY estão definidas.'
